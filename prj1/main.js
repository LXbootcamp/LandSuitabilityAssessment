...
function makeWFSSource2(clickedFeatureJibun) {
  vectorSource3 = new VectorSource({
    url: encodeURI(
      "http://localhost:42888/geoserver/campWS/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=campWS:muan_all2&outputFormat=application/json&CQL_FILTER=" +
...
function sendData(clickedFeatureA1, insertedA20) {
  var url =
    "./fetchData.jsp?selectedA1=" +
    encodeURIComponent(clickedFeatureA1) +
    "&insertedA20=" +
    encodeURIComponent(insertedA20);
  fetch(url, {
...
function calculateDevVal() {
  ...
  const bbbbValues = ["bbbb01", "bbbb02", "bbbb03", "bbbb04", "bbbb05"].map(
    (id) => parseFloat(document.getElementById(id).value) || 0
  );
...
["bbbb01", "bbbb02", "bbbb03", "bbbb04", "bbbb05"].forEach((id) => {
  document.getElementById(id).addEventListener("input", calculateDevVal);
});
